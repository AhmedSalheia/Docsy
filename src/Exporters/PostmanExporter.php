...
class PostmanExporter extends AbstractExporter
{
    public static function export(Docsy $docsy, $collection = ""): string
    {
        ...
        return json_encode(self::transformCollection($docsy->getCollection($collection)),true ? JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES : 0);
    }
    ...
    protected static function transformRequestUrl(string $base_url, array $path, array $queryParams = [], array $pathParams = []): array
    {
    ...
    protected static function transformRequestHeaders(array $headers): array
    {
        if (empty($headers)) {
            return [];
        }
